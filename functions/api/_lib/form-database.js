...
export async function handleFormDatabaseSubmission({ request, env, formType }) {
  const form = FORM_DATABASE_FORMS[formType];
...
export async function handleFormRequestPost({ request, env, formType }) {
  try {
    return await handleFormDatabaseSubmission({ request, env, formType });
  } catch (error) {
    return jsonResponse({
      ok: false,
...
export function handleFormRequestGet() {
