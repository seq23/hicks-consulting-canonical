...
function candidateWarnings(candidate) {
  ...
  if (candidate.publishMode !== 'full_safe_autonomy') warnings.push('autonomy mode missing');
  if (candidate.routineApprovalRequired !== false) warnings.push('routine approval still enabled');
  return warnings;
}
...
function renderGeneratedCandidates() {
  ...
  tbody.innerHTML = GENERATED_CANDIDATES.map(item => {
    ...
    return `<tr>
      ...
      <td class="small">Automatic next step: the system admits, drafts, validates, repairs, and schedules this item inside an existing content slot. If facts or safety evidence are missing, it records the exception instead of publishing.</td>
    </tr>`;
  }).join('');
}
