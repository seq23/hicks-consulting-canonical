...
function requireDigitalProductsAdmin(request, env) {
  ...
  if (!expected) {
    return { ok: false, response: jsonResponse({ ok: false, error: 'Digital products admin token is not configured.' }, 503) };
  }
  const actual = request.headers.get('x-admin-token') || '';
  if (actual !== expected) {
    return { ok: false, response: jsonResponse({ ok: false, error: 'Digital products admin token did not match.' }, 401) };
  }
  return { ok: true };
}
...
async function readDigitalProductsSeedCatalog(request, env) {
  ...
  if (env.ASSETS && typeof env.ASSETS.fetch === 'function') {
    ...
    if (assetResponse && assetResponse.ok) {
      return assetResponse.json().catch(() => ({ schemaVersion: '1.0.0', products: [] }));
    }
  }
...
async function readDigitalProductsCatalog(request, env) {
  if (env.DIGITAL_PRODUCTS_KV && typeof env.DIGITAL_PRODUCTS_KV.get === 'function') {
    ...
    if (stored && Array.isArray(stored.products)) {
      return { ...stored, apiConfigured: true };
    }
  }
...
async function writeDigitalProductsCatalog(env, catalog) {
  if (!env.DIGITAL_PRODUCTS_KV || typeof env.DIGITAL_PRODUCTS_KV.put !== 'function') {
    return {
      ok: false,
      response: jsonResponse({ ok: false, error: 'DIGITAL_PRODUCTS_KV binding is required before product changes can persist.' }, 503)
    };
  }
...
async function digitalProductFromForm(request, env) {
  ...
  const pdfUrl = await uploadDigitalProductFile(env, pdf, `digital-products/${id}/source`);
  const coverUrl = await uploadDigitalProductFile(env, cover, `digital-products/${id}/cover`);
...
function validateDigitalProductForPublication(product) {
  ...
  if (!product.title || !product.description) {
    return 'Published products require title and description.';
  }

  if (product.productType === 'free' && !product.downloadUrl) {
    return 'Published free downloads require a PDF download URL.';
  }
...
function mergeDigitalProduct(existing, incoming) {
  ...
  if ((!incoming.coverSource || incoming.coverSource === 'branded_placeholder') && existing && existing.coverSource && existing.coverSource !== 'branded_placeholder') {
    merged.coverSource = existing.coverSource;
  }

  if (merged.productType === 'premium' && merged.status === 'published' && merged.gumroadUrl && merged.gumroadUrl !== 'https://www.gumroad.com') {
...
async function handleDigitalProductPublish(request, env) {
  ...
  const catalog = await readDigitalProductsCatalog(request, env);
  const products = (catalog.products || []).map((item) => item.id === id
    ? { ...item, status: 'published', checkoutStatus: item.productType === 'premium' ? 'live' : item.checkoutStatus, publishedAt: new Date().toISOString().slice(0, 10) }
    : item
  );
...
async function handleDigitalProductsRequest(request, env, url) {
  ...
  if (url.pathname === '/api/digital-products/update') {
    if (request.method !== 'POST') return jsonResponse({ ok: false, error: 'Method not allowed.' }, 405);
    return handleDigitalProductUpdate(request, env);
  }

  if (url.pathname === '/api/digital-products/publish') {
    if (request.method !== 'POST') return jsonResponse({ ok: false, error: 'Method not allowed.' }, 405);
    return handleDigitalProductPublish(request, env);
  }

  if (url.pathname === '/api/digital-products/revoke') {
    if (request.method !== 'POST') return jsonResponse({ ok: false, error: 'Method not allowed.' }, 405);
    return handleDigitalProductRevoke(request, env);
  }

  if (url.pathname === '/api/digital-products/cover') {
    return jsonResponse({
      ok: true,
      coverPolicy: 'Custom cover image is preferred. If no cover image is uploaded, product cards use the uploaded PDF first-page preview when a PDF URL exists.'
    });
  }

  if (url.pathname === '/api/digital-products/upload') {
    return jsonResponse({ ok: false, error: 'Use /api/digital-products/update for combined metadata and file upload.' }, 405);
  }

  if (url.pathname.startsWith('/api/digital-products/file/')) {
    if (request.method !== 'GET') return jsonResponse({ ok: false, error: 'Method not allowed.' }, 405);
    return handleDigitalProductFile(request, env, url);
  }
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (digitalProductsResponse) {
        return digitalProductsResponse;
      }
